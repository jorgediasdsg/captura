...
captura(){
	...
	while read site link; do 				#Abre o laço de execução dos screenshots.
		...
		wget -q "$(cat $HOME/google2.cfg)&entry.1481241936=$site&entry.338287161=$diaatual&entry.1056328059=$valor&entry.196043540=$variacao&entry.1268065653=$porcentagem&entry.1463811894=$h&entry.80433284=$porcentagem_fechamento&entry.493625541=$data_fechamento&entry.1887060639=$origem" -O "$site2.html"
