...
captura(){
	...
	while read site link; do 				#Abre o laço de execução dos screenshots.
		...
		porcentagem_fechamento=$(grep "\<td\> class=\"bold" $site.html | head -1 | sed 's/<\/td>.*// ; s/.*\">//')
		data_fechamento=$(grep "data-real-valu" $site.html | head -1 | sed "s/<\/td>.*// ; s/.*\">//" | sed "s/ var.*//" | xargs -n 5 | sed "s/\./\// ; s/\./\//")
